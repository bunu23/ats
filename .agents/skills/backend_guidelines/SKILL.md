...
2. **Data Access:** All database queries must go through the helper functions in `src/lib/db.ts`. Do not write raw queries in the API controllers.
3. **Date Formatting:** All dates must be stored and handled as ISO 8601 strings.
4. **Naming Conventions**: All utility and helper files must use `camelCase` (e.g., `formatDate.ts`).
